...
private:
     ...
     */
    auto get_iommu_group(const std::string& pci_addr) -> int {
...
public:
    ...
    explicit PorthVFIODevice(const std::string& pci_addr) {
        ...
        if (m_group_fd < 0) {
            throw std::runtime_error(std::format("Failed to open IOMMU group {}.", group_id));
        }
    ...
    ~PorthVFIODevice() {
        if (m_bar_ptr && m_bar_ptr != MAP_FAILED)
     ...
     */
    auto map_dma(void* vaddr, size_t size) -> uint64_t {
    ...
    // Prohibit copying to prevent multi-owner conflicts over hardware BARs
